...
bool PosixWrite(int fd, const char* buf, size_t nbyte, Env::IOPriority pri) {
  const size_t kLimitProperSize = 1UL << 19;

  const char* src = buf;
  size_t left = nbyte;

  while (left != 0) {
    size_t bytes_to_write = std::min(left, kLimitProperSize);

    ssize_t done;
    if (io_controller.status().ok()) {
      done = io_controller.AddJob(fd, src, bytes_to_write, pri);
    } else {
...
IOStatus PosixWritableFile::Append(const Slice& data, const IOOptions& /*opts*/,
  ...
  if (!PosixWrite(fd_, src, nbytes, io_priority_)) {
