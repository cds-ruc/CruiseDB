...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  ...
  // std::cout<<"Schedule Compaction:\tScheduled: "<<bg_compaction_scheduled_<<"\tUnscheduled: "<<unscheduled_compactions_<<std::endl;
  
  // if (bg_compaction_scheduled_ == 0 && bg_manual_compaction_scheduled_ == 0 ) {
  //   CompactionArg* ca = new CompactionArg;
  //   ca->db = this;
  //   ca->prepicked_compaction = nullptr;
  //   bg_manual_compaction_scheduled_++;
  //   env_->Schedule(&DBImpl::BGWorkManualCompaction, ca, Env::Priority::LOW, this,
  //                  &DBImpl::UnscheduleManualCompactionCallback);
  // } else if (bg_manual_compaction_scheduled_ < 0) {
  //   bg_manual_compaction_scheduled_ = 0;
  // }
}
...
void DBImpl::BGWorkManualCompaction(void* arg) {
  CompactionArg ca = *(reinterpret_cast<CompactionArg*>(arg));
  delete static_cast<CompactionArg*>(arg);
  IOSTATS_SET_THREAD_POOL_ID(Env::Priority::LOW);
  ca.db->BackgroundCallManualCompaction();
}
...
void DBImpl::UnscheduleManualCompactionCallback(void* arg) {
  delete reinterpret_cast<CompactionArg*>(arg);
  std::cout << "BGWorkManualCompaction Unscheduled" << std::endl;
}
...
void DBImpl::BackgroundCallManualCompaction() {
  //const int usleep_time = 1000;
  static int bg_manual_compaction_call_times(0);
  static int bg_manual_compaction_run_times(0);
  auto cfh = DefaultColumnFamily();
  auto cfd = reinterpret_cast<ColumnFamilyHandleImpl*>(cfh)->cfd();
  {
    InstrumentedMutexLock l(&mutex_);

    ++bg_manual_compaction_call_times;
    ...
    const std::vector<FileMetaData*>& files0 = vstorage -> LevelFiles(0);
    for (unsigned j = 0; file_names.empty() && j < files0.size(); ++j) {
      if (files0[j]->being_compacted) continue;
      file_names.push_back(MakeTableFileName(files0[j]->fd.GetNumber()));
      output_level = 1;
    }

    for (int i = 0; file_names.empty() && i <= vstorage->MaxInputLevel(); ++i) {
      int input_level = vstorage->CompactionScoreLevel(i);
      const std::vector<FileMetaData*>& files = vstorage->LevelFiles(input_level);
      for (unsigned j=0; file_names.empty() && j < files.size(); ++j) {
        if (files[j]->being_compacted) continue;
        file_names.push_back(MakeTableFileName(files[j]->fd.GetNumber()));
        output_level = input_level + 1;
      }
    }

    if (!file_names.empty()) {
      auto* mcfoptions = cfd->GetLatestMutableCFOptions();
      auto* icfoptions = cfd->ioptions();
      CompactionOptions cptoptions;
      cptoptions.compression = kDisableCompressionOption;
      cptoptions.output_file_size_limit = MaxFileSizeForLevel(*mcfoptions, output_level, icfoptions->compaction_style, vstorage->base_level(), icfoptions->level_compaction_dynamic_level_bytes);
      ++bg_manual_compaction_run_times;

      mutex_.Unlock();
      CompactFiles(cptoptions, cfh, file_names, output_level);
      mutex_.Lock();
    } else {
      //usleep(usleep_time);
      bg_manual_compaction_scheduled_ = 0;
    ...
    if (true || bg_manual_compaction_call_times % 10 == 0) {
      std::cout<<"bg_manual_compaction: "<<bg_manual_compaction_run_times<<" / "<<bg_manual_compaction_call_times<<std::endl;
    }
